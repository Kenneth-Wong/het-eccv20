# ---------------------------------------------------------------
# train_GCNcaptions.py
# Set-up time: 2020/2/20 上午10:10
...
if conf.model == 'motifnet':
    ...
    raise ValueError()
from lib.caption_lstm.GCNLSTMModel import GCNLSTMModel
...
detector = RelModel(classes=train.ind_to_classes, rel_classes=train.ind_to_predicates,
                    ...
                    gcn_caption=conf.gcn_captioning,
                    )
...
def train_epoch(epoch_num):
    ...
    # update training params before every epoch start
    if epoch_num > conf.lr_decay_start and conf.lr_decay_start >= 0:
        frac = (epoch_num - conf.lr_decay_start) // conf.lr_decay_every
        decay_factor = conf.lr_decay_rate ** frac
        lr = conf.lr * decay_factor
        for group in optimizer.param_groups:
            group['lr'] = lr
    if epoch_num > conf.scheduled_sampling_start and conf.scheduled_sampling_start >= 0:
        frac = (epoch_num - conf.scheduled_sampling_start) // conf.scheduled_sampling_increase_every
        ss_prob = min(conf.scheduled_sampling_increase_prob * frac, conf.scheduled_sampling_max_prob)
        captionGenerator.ss_prob = ss_prob

    for b, batch in enumerate(train_loader):
        train_batch(epoch_num, b, batch, verbose=b % (conf.print_interval * 10) == 0)


def train_batch(epoch, count, b, verbose=False):
    captionGenerator.train()
    start = time.time()
    result = detector[b]

    # get the result from scene graph model
    im_inds, region_feats, pred_classes, rels, obj_classes, seq_labels, mask_labels, coco_ids = result

    optimizer.zero_grad()
    outputs = captionGenerator(im_inds, region_feats, pred_classes, rels, obj_classes, seq_labels, mask_labels,)
    loss = crit(outputs, seq_labels[:, 1:], mask_labels[:, 1:])
    ...
    if verbose:
        print("iter {} (epoch {}), train loss = {:.3f}, time/batch = {:.3f}".format(count, epoch, train_loss,
                                                                                    end - start))
...
def val_epoch():
    ...
    for val_b, batch in enumerate(test_loader):
        ...
        seq, _ = captionGenerator.sample(im_inds, region_feats, pred_classes, rels, obj_classes,
    ...
    # dump predictions
    if conf.eval_dump:
        json.dump(predictions, open(conf.save_dir + '_print_predictions.json', 'w'))

    lang_stats = language_eval(predictions, test_coco_ids, conf.save_dir + '_cache.json')

    return loss_sum / loss_evals, predictions, lang_stats


print("Training starts now!")
optimizer = get_optim(captionGenerator, conf.lr)
...
for epoch in range(start_epoch + 1, start_epoch + 1 + conf.num_epochs):
    ...
    if conf.save_dir is not None:
        torch.save({
            'epoch': epoch,
            'state_dict': captionGenerator.state_dict(),
        }, os.path.join(conf.save_dir, '{}-{}.tar'.format('vgcaption', epoch)))

    val_loss, predictions, lang_stats = val_epoch()
    print("val (epoch {}), val loss = {:.3f}".format(epoch, val_loss))
    ...
    if current_score > best_score:
        ...
        torch.save({
            'epoch': epoch,
            'state_dict': captionGenerator.state_dict(),
        }, os.path.join(conf.save_dir, '{}-best.tar'.format('vgcaption')))
    print('**** best epoch: {}, \t best CIDEr: {:.4f} *****'.format(best_epoch, best_score))
