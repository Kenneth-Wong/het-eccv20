...
from config import VG_IMAGES, IM_DATA_FN, VG_SGG_FN, VG_SGG_DICT_FN, BOX_SCALE, IM_SCALE, PROPOSAL_FN, SALIENCY_FN, DEPTH_FN
...
class VG(Dataset):
    def __init__(self, mode, roidb_file=VG_SGG_FN, dict_file=VG_SGG_DICT_FN,
        ...
        self.sal_transform_pipeline = Compose([Resize(int(IM_SCALE / 16)),
                                               ToTensor()])
    ...
    def __getitem__(self, index):
        ...
        # load salinecy map
        saliency_unpadded = self.sal_h5[self.image_index[index]]  # 512, 512
        saliency_unpadded = Image.fromarray(saliency_unpadded)

        # load depth map
        depth_unpadded = self.depth_h5[self.image_index[index]]  # 512, 512
