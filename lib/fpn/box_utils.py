...
def bbox_intersections_pair(box_a, box_b):
    A = box_a.size(0)
    B = box_b.size(0)
    assert A == B
...
def bbox_overlaps_pair(box_a, box_b):
    inter = bbox_intersections_pair(box_a, box_b)
    area_a = ((box_a[:, 2] - box_a[:, 0] + 1.0) *
              (box_a[:, 3] - box_a[:, 1] + 1.0))  # A
    area_b = ((box_b[:, 2] - box_b[:, 0] + 1.0) *
              (box_b[:, 3] - box_b[:, 1] + 1.0))  # A
    union = area_a + area_b - inter
    return inter / union  # A

def co_bbox_overlaps(box_a, box_b):
    """Compute the jaccard overlap of two sets of boxes.  The jaccard overlap
        is simply the intersection over union of two boxes.  Here we operate on
        ground truth boxes and default boxes.
        E.g.:
    ...
    inter_sub = bbox_intersections(box_a_sub, box_b_sub)
    area_a_sub = ((box_a_sub[:, 2] - box_a_sub[:, 0] + 1.0) *
                  (box_a_sub[:, 3] - box_a_sub[:, 1] + 1.0)).unsqueeze(1).expand_as(inter_sub)  # [A,B]
    area_b_sub = ((box_b_sub[:, 2] - box_b_sub[:, 0] + 1.0) *
                  (box_b_sub[:, 3] - box_b_sub[:, 1] + 1.0)).unsqueeze(0).expand_as(inter_sub)  # [A,B]
    union_sub = area_a_sub + area_b_sub - inter_sub

    inter_obj = bbox_intersections(box_a_obj, box_b_obj)
    area_a_obj = ((box_a_obj[:, 2] - box_a_obj[:, 0] + 1.0) *
                  (box_a_obj[:, 3] - box_a_obj[:, 1] + 1.0)).unsqueeze(1).expand_as(inter_obj)  # [A,B]
    area_b_obj = ((box_b_obj[:, 2] - box_b_obj[:, 0] + 1.0) *
                  (box_b_obj[:, 3] - box_b_obj[:, 1] + 1.0)).unsqueeze(0).expand_as(inter_obj)  # [A,B]
    union_obj = area_a_obj + area_b_obj - inter_obj

    #return (inter_sub + inter_obj) / (union_sub + union_obj)

    iou_sub = bbox_overlaps(box_a_sub, box_b_sub)
    iou_obj = bbox_overlaps(box_a_obj, box_b_obj)
    return iou_sub * iou_obj
