...
def rel_assignments(im_inds, rpn_rois, roi_gtlabels, gt_boxes, gt_classes, gt_rels, image_offset,
    ...
    for im_ind in range(num_im):
        ...
        for i, (from_gtind, to_gtind, rel_id) in enumerate(gt_rels_i):
            ...
            for rel_to_add in npr.choice(p.shape[0], p=p, size=num_to_add, replace=False):
                ...
                rels_to_gt_rel.append(i + num_gt_rel_seen)

        fg_rels = np.array(fg_rels, dtype=np.int64)
        all_fg_rels = fg_rels
        rels_to_gt_rel = np.array(rels_to_gt_rel, dtype=np.int64)

        if fg_rels.size > 0 and fg_rels.shape[0] > fg_rels_per_image:
            rand_idx = npr.choice(fg_rels.shape[0], size=fg_rels_per_image, replace=False)
            fg_rels = fg_rels[rand_idx]
            rels_to_gt_rel = rels_to_gt_rel[rand_idx]
        elif fg_rels.size == 0:
            fg_rels = np.zeros((0, 3), dtype=np.int64)
            all_fg_rels = np.zeros((1, 3), dtype=np.int64)
            rels_to_gt_rel = -1 * np.ones(1, dtype=np.int64)
        ...
        if bg_rels.size > 0:
            ...
            rels_to_gt_rel = np.concatenate((rels_to_gt_rel, np.ones(bg_rels.shape[0], dtype=np.int64)*-1), 0)
        else:
        ...
        rels_to_gt_rel = rels_to_gt_rel[np.lexsort((all_rels_i[:, 1], all_rels_i[:, 0]))]
        ...
        rels_to_gt_rel_labels.append(rels_to_gt_rel)
        num_box_seen += pred_boxes_i.shape[0]
        num_gt_rel_seen += gt_rels_i.shape[0]
    ...
    fg_rel_labels = torch.LongTensor(np.concatenate(fg_rel_labels, 0)).cuda(rpn_rois.get_device(),
                                                                      async=True)
    rels_to_gt_rel_labels = torch.LongTensor(np.concatenate(rels_to_gt_rel_labels, 0)).cuda(rpn_rois.get_device(),
                                                                                            async=True)
    return rel_labels, fg_rel_labels, rels_to_gt_rel_labels
