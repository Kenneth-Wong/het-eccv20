...
def proposal_assignments_gtbox(rois, gt_boxes, gt_classes, gt_rels, image_offset, fg_thresh=0.5):
    ...
    rels_to_gt = torch.arange(0, fg_rels.size(0)).long().cuda(gt_rels.get_device())
    ...
    if num_fg < fg_rels.size(0):
        fg_rels, rand_idx = random_choose(fg_rels, num_fg, True)
        rels_to_gt = rels_to_gt[rand_idx].contiguous()
    ...
    if num_bg > 0:
        ...
        rels_to_gt = torch.cat((rels_to_gt, (-1 * torch.ones(num_bg).long()).cuda(rels_to_gt.get_device())), 0)
    else:
        rel_labels = fg_rels
    ...
    return rois, labels, rel_labels, rels_to_gt
