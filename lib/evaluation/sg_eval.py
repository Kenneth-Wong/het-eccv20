...
def get_treeNodes_depth(depth_dict, root):
    depth_dict[root.index] = root.depth()
    for child in root.children:
        get_treeNodes_depth(depth_dict, child)
...
class BasicSceneGraphEvaluator:
    def __init__(self, mode, multiple_preds=False, num_predicates=50):
        self.result_dict = {}
        self.result_dict2 = {}
        self.gtrel_dist_dict = defaultdict(int)  # gtrel distribution on the tree
        self.gtkeyrel_dist_dict = defaultdict(int)  # gtkeyrel distribution on the tree
        self.predrel_dist_dict = {}
        self.predrel_treedeep_scores_dict = {}
        self.predmatchedrel_dist_dict = {}
        self.predmatchedkeyrel_dist_dict = {}

        self.mode = mode
        self.result_dict[self.mode + '_recall'] = {1: [], 5: [], 20: [], 50: [], 100: []}
        self.result_dict[self.mode + '_key_recall'] = {1: [], 5: [], 20: [], 50: [], 100: []}
        self.result_dict2[self.mode + '_recall'] = {1: [], 5: [], 20: [], 50: [], 100: []}
        self.result_dict2[self.mode + '_key_recall'] = {1: [], 5: [], 20: [], 50: [], 100: []}
        self.predrel_dist_dict[self.mode] = defaultdict(int)
        self.predrel_treedeep_scores_dict[self.mode] = defaultdict(list)
        self.predmatchedrel_dist_dict[self.mode] = defaultdict(int)
        self.predmatchedkeyrel_dist_dict[self.mode] = defaultdict(int)
    ...
    def evaluate_scene_graph_entry(self, gt_entry, pred_scores, viz_dict=None, iou_thresh=0.5):
        res = evaluate_from_dict(gt_entry, pred_scores, self.mode, self.result_dict, self.result_dict2,
                                 self.gtrel_dist_dict, self.gtkeyrel_dist_dict, self.predrel_dist_dict,
                                 self.predmatchedrel_dist_dict, self.predmatchedkeyrel_dist_dict,
                                 predrel_treedeep_scores_dict=self.predrel_treedeep_scores_dict,
        ...
        assert isinstance(tree_width, int)
        self.tree_depths.append(tree_depth)
        self.tree_widths.append(tree_width)
        # self.print_stats()
        return res
    ...
    def print_stats(self):
        ...
        for k, v in self.result_dict[self.mode + '_recall'].items():
            ...
            print('key-R@%i: %f' % (k, np.mean(self.result_dict[self.mode + '_key_recall'][k])))
            # if self.mode+'_recall_hit' in self.result_dict:
            #    print(self.result_dict[self.mode + '_recall_hit'][k])
            # if self.mode + '_recall_count' in self.result_dict:
            ...
            if self.mode + '_key_recall_hit' in self.result_dict and self.mode + '_key_recall_count' in self.result_dict:
                avg = 0
                for idx in range(self.num_predicates):
                    tmp_avg = float(self.result_dict[self.mode + '_key_recall_hit'][k][idx + 1]) / float(
                        self.result_dict[self.mode + '_key_recall_count'][k][idx + 1] + 1e-10)
                    avg += tmp_avg
                    # print(str(idx+1), ' ', tmp_avg)
                print('key cls avg', avg / (1.0 * self.num_predicates))
                print('key total R', float(self.result_dict[self.mode + '_key_recall_hit'][k][0]) / float(
                    self.result_dict[self.mode + '_key_recall_count'][k][0] + 1e-10))

        print('~~~~~~~~~~~~~~~~~~~~~~' + 'proposal match' + '~~~~~~~~~~~~~~~~~~~~~~~~~')
        for k, v in self.result_dict2[self.mode + '_recall'].items():
            print('R@%i: %f' % (k, np.mean(v)))
            print('key-R@%i: %f' % (k, np.mean(self.result_dict2[self.mode + '_key_recall'][k])))
            # if self.mode+'_recall_hit' in self.result_dict:
            #    print(self.result_dict[self.mode + '_recall_hit'][k])
            # if self.mode + '_recall_count' in self.result_dict:
            #    print(self.result_dict[self.mode + '_recall_count'][k])
            if self.mode + '_recall_hit' in self.result_dict2 and self.mode + '_recall_count' in self.result_dict2:
                avg = 0
                for idx in range(self.num_predicates):
                    tmp_avg = float(self.result_dict2[self.mode + '_recall_hit'][k][idx + 1]) / float(
                        self.result_dict2[self.mode + '_recall_count'][k][idx + 1] + 1e-10)
                    avg += tmp_avg
                    # print(str(idx+1), ' ', tmp_avg)
                print('cls avg', avg / (1.0 * self.num_predicates))
                print('total R', float(self.result_dict2[self.mode + '_recall_hit'][k][0]) / float(
                    self.result_dict2[self.mode + '_recall_count'][k][0] + 1e-10))

            if self.mode + '_key_recall_hit' in self.result_dict2 and self.mode + '_key_recall_count' in self.result_dict2:
                avg = 0
                for idx in range(self.num_predicates):
                    tmp_avg = float(self.result_dict2[self.mode + '_key_recall_hit'][k][idx + 1]) / float(
                        self.result_dict2[self.mode + '_key_recall_count'][k][idx + 1] + 1e-10)
                    avg += tmp_avg
                    # print(str(idx+1), ' ', tmp_avg)
                print('key cls avg', avg / (1.0 * self.num_predicates))
                print('key total R', float(self.result_dict2[self.mode + '_key_recall_hit'][k][0]) / float(
                    self.result_dict2[self.mode + '_key_recall_count'][k][0] + 1e-10))

        if self.mode == 'sgcls' or self.mode == 'predcls':
            print('~~~~~~~~~~~~~~~~~~~~~~' + 'tree distribution' + '~~~~~~~~~~~~~~~~~~~~~~~~~')

            print('gt rel distribution (node depth / normalized num):')
            total = sum(list(self.gtrel_dist_dict.values()))
            for k, v in self.gtrel_dist_dict.items():
                print(k, v / total)

            print('gt key rel distribution (node depth / normalized num):')
            total = sum(list(self.gtkeyrel_dist_dict.values()))
            for k, v in self.gtkeyrel_dist_dict.items():
                print(k, v / total)

            print('pred rel distribution (node depth / normalized num):')
            total = sum(list(self.predrel_dist_dict[self.mode].values()))
            for k, v in self.predrel_dist_dict[self.mode].items():
                print(k, v / total)

            print('pred rel treepdepth-scores:')
            for k, v in self.predrel_treedeep_scores_dict[self.mode].items():
                print(k, np.mean(v), np.std(v), len(v))

            print('pred matched rel distribution (node depth / normalized num):')
            total = sum(list(self.predmatchedrel_dist_dict[self.mode].values()))
            for k, v in self.predmatchedrel_dist_dict[self.mode].items():
                print(k, v / total)

            print('pred matched key rel distribution (node depth / normalized num):')
            total = sum(list(self.predmatchedkeyrel_dist_dict[self.mode].values()))
            for k, v in self.predmatchedkeyrel_dist_dict[self.mode].items():
                print(k, v / total)

            print('~~~~~~~~~~~~~~~~~~~~~~' + 'tree depths avg' + '~~~~~~~~~~~~~~~~~~~~~~~~~')
            if len(self.tree_depths) > 0:
                print('tree depths avg: %.5f' % (np.mean(self.tree_depths)))
            print('~~~~~~~~~~~~~~~~~~~~~~' + 'tree widths avg' + '~~~~~~~~~~~~~~~~~~~~~~~~~')
            if len(self.tree_widths) > 0:
                print('tree widths avg: %.5f' % (np.mean(self.tree_widths)))
...
def evaluate_recall(gt_rels, gt_boxes, gt_classes,
    ...
    return pred_to_gt, subobj_to_gt, pred_5ples, relation_scores
