...
def random_choose(tensor, num, return_idx=False):
    ...
    if num_choose == tensor.size(0):
        if return_idx:
            return tensor, torch.LongTensor(np.arange(num_choose)).cuda(tensor.get_device())
        else:
            return tensor
...
def clip_grad_norm(named_parameters, max_norm, clip=False, verbose=False):
    ...
    if verbose:
        ...
        for name, norm in sorted(param_to_norm.items(), key=lambda x: -x[1]):
            print("{:<100s}: {:.3f}, ({})".format(name, norm, param_to_shape[name]))
        print('-------------------------------', flush=True)
...
def keyrel_loss(rel_rank_scores, keyrel_labels, sample_num=512, margin=0.2):
    """

    :param rel_rank_scores: N,
    :param keyrel_labels: N, 2
    :return:
    """
    im_inds = (keyrel_labels[:, 0]).data
    num_im = int(im_inds[-1] + 1)

    is_cand = (im_inds[:, None] == im_inds[None]) # N, N, filter out not in different images
    is_cand.view(-1)[diagonal_inds(is_cand)] = 0

    # sample the key and non-key rel
    keyrel_inds = torch.nonzero(keyrel_labels[:, 1] == 1).view(-1)
    nonkeyrel_inds = torch.nonzero(keyrel_labels[:, 1] == 0).view(-1)
    keyrel_inds_np = keyrel_inds.data.cpu().numpy()
    nonkeyrel_inds_np = nonkeyrel_inds.data.cpu().numpy()
    pos_neg = torch.LongTensor(np.array(list(product(keyrel_inds_np, nonkeyrel_inds_np)))).cuda(rel_rank_scores.get_device())
    pos_neg_cand = torch.ByteTensor(im_inds.shape[0], im_inds.shape[0]).fill_(0).cuda(rel_rank_scores.get_device())
    pos_neg_cand.view(-1)[pos_neg[:, 0] * im_inds.shape[0] + pos_neg[:, 1]] = 1
    pos_neg_cand = pos_neg_cand & is_cand
    pos_neg_pair_inds = torch.nonzero(pos_neg_cand)
    pos_neg_pair_inds = random_choose(pos_neg_pair_inds, sample_num)
    loss = F.margin_ranking_loss(torch.index_select(rel_rank_scores, 0, Variable(pos_neg_pair_inds[:, 0])),
                                 torch.index_select(rel_rank_scores, 0, Variable(pos_neg_pair_inds[:, 1])),
                                 target=Variable(torch.FloatTensor(pos_neg_pair_inds.shape[0]).fill_(1).cuda(rel_rank_scores.get_device())),
                                 margin=margin)
    return loss
...
class FocalLoss(nn.Module):
    def __init__(self, alpha, gamma, size_average=True):
        ...
        if isinstance(alpha, (float, int)):
            self.alpha = torch.Tensor([alpha, 1-alpha])  # binary classification
    ...
    def forward(self, input, target):
        ...
        logpt = F.log_softmax(input)  # N, C
        logpt = logpt.gather(1, target).view(-1)  # N,
...
def get_area_maps(shape, im_inds, boxes, scale):
    h, w = shape

    num_img = int(im_inds[-1]) + 1
    areas = (boxes[:, 2] - boxes[:, 0] + 1) * (boxes[:, 3] - boxes[:, 1] + 1)
    areas_ratios = areas / (h * w * scale**2)
    area_maps = torch.FloatTensor(num_img, 1, h*scale, w*scale).cuda(im_inds.get_device()).fill_(0)
    for i, start, end in enumerate_by_image(im_inds):
        this_boxes = boxes[start:end]
        this_boxes_int32 = this_boxes.long()
        this_area_ratios = areas_ratios[start:end]

        num_boxes = end + start + 1
        this_area_maps = torch.FloatTensor(num_boxes, h*scale, w*scale).cuda(im_inds.get_device()).fill_(2)  # a trick
        for ind, box in enumerate(this_boxes_int32):
            box = box.view(-1)
            this_area_maps[ind, box[1]:(box[3]+1), box[0]:(box[2]+1)] = this_area_ratios[ind]

        this_area_maps, _ = torch.min(this_area_maps, dim=0)  # h,w
        no_value_pos = torch.nonzero(this_area_maps == 2)
        if len(no_value_pos) > 0:
            this_area_maps[no_value_pos[:, 0], no_value_pos[:, 1]] = 0
        area_maps[i, 0, :, :] = this_area_maps

    return Variable(area_maps)
